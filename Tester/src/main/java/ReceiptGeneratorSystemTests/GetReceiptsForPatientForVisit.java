...
public class GetReceiptsForPatientForVisit {
    ...
    @Test
    public void GetReceiptsForPatientForVisit() {
        //Response response =
        when()
                .get(_Utils.baseURIReceipts + "/patient/" + receipt.get("patientId").toString().trim() + "/visit/" + receipt.get("visitId").toString().trim())
                .then()
                .statusCode(200)
                .assertThat()
                .body("[0]", hasKey("id"))
